...
def test_memory_policy_proper_split(X, y, policy_name, memory_class=None, update_method='fifo'):
    """
    CRITICAL FIX: Proper data split ensuring anomalies in test set
    ...
    train_size = min(256, len(normal_idx) // 4)
    train_idx = normal_idx[:train_size]
    
    # Test on: remaining normals + ALL anomalies
    test_normal_idx = normal_idx[train_size:train_size+1000]
    test_anomaly_idx = anomaly_idx  # ALL anomalies
    ...
    if memory_class is None:
        ...
        if policy_name == 'Weighted FIFO':
            memory = memory_class(memory_size, encoding_dim, k_neighbors=3, gamma=0.0, recency_weight=0.7)
        else:
...
def run_ablation_study():
    ...
    # Load data
    X, y = load_kddcup99_corrected(sample_size=10000, max_features=100)
